...
do
  ...
  echo ./node_modules/node.io/bin/node.io -s save "https://www.youversion.com/en-GB/bible/"$4"/"$3"."$i"."$5 | \
  ...
  ./node_modules/node.io/bin/node.io -s save "https://www.youversion.com/en-GB/bible/"$4"/"$3"."$i"."$5 | \
    grep div\ class\=\"label \
    | sed -e 's/L<\/span><span class="sc"><span class="content">ord<\/span>/Lord/g' \
    | sed -e 's/\([^0-9]\)<\/span><span class="\(content\)">/\1     /g' >>  $6
